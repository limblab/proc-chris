...
%% configure bin parameters:
    ...
    ex.binConfig.include(1).which=find([ex.units.data.ID]>-1 & [ex.units.data.ID]<255);
    ex.binConfig.include(2).field = 'analog';
    ex.binConfig.include(3).field = 'emg';
    ex.binConfig.include(4).field='kin';
    ex.binConfig.include(4).which={};%empty gets you all columns, a cell array of strings only pulls the columns with the specified labels
    ex.binConfig.include(5).field='force';
...
%     %used
%     window1 = [[ex.trials.data.bumpTime(rewardTrials)], [ex.trials.data.bumpTime(rewardTrials)]+.2];
