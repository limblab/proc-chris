...
%% 
close all
upBumpKin = zeros(length(upMove), length(upMove(1).idx_peak_speed-(before*100):upMove(1).idx_peak_speed+(after*100)), 2);
for  i = 1:length(upMove)
    upBumpKin(i,:,:) = upMove(i).vel(upMove(i).idx_peak_speed-(before*100):upMove(i).idx_peak_speed+(after*100),:);
    
end
meanUpKin = squeeze(mean(upBumpKin));
speedUpKin = sqrt(meanUpKin(:,1).^2 + meanUpKin(:,2).^2);

downBumpKin = zeros(length(upMove), length(upMove(1).idx_peak_speed-(before*100):upMove(1).idx_peak_speed+(after*100)), 2);
for  i = 1:length(downMove)
    downBumpKin(i,:,:) = downMove(i).vel(downMove(i).idx_peak_speed-(before*100):downMove(i).idx_peak_speed+(after*100),:);
    
end
meanDownKin = squeeze(mean(downBumpKin));
speedDownKin = sqrt(meanDownKin(:,1).^2 + meanDownKin(:,2).^2);

leftBumpKin = zeros(length(upMove), length(upMove(1).idx_peak_speed-(before*100):upMove(1).idx_peak_speed+(after*100)), 2);
for  i = 1:length(leftMove)
    leftBumpKin(i,:,:) = leftMove(i).vel(leftMove(i).idx_peak_speed-(before*100):leftMove(i).idx_peak_speed+(after*100),:);
    
end
meanleftKin = squeeze(mean(leftBumpKin));
speedleftKin = sqrt(meanleftKin(:,1).^2 + meanleftKin(:,2).^2);

rightBumpKin = zeros(length(upMove), length(upMove(1).idx_peak_speed-(before*100):upMove(1).idx_peak_speed+(after*100)), 2);
for  i = 1:length(rightMove)
    rightBumpKin(i,:,:) = rightMove(i).vel(rightMove(i).idx_peak_speed-(before*100):rightMove(i).idx_peak_speed+(after*100),:);
end
...
for num1 = unitsToPlot
    ...
    xlabel('time (ms)')
    for  i = 1:length(upMove)
        upBumpFiring(i,:) = upMove(i).Cuneate_spikes(upMove(i).idx_peak_speed-(before*100):upMove(i).idx_peak_speed+(after*100),num1);
    end
    ...
    for  i = 1:length(downMove)
        downBumpFiring(i,:) = downMove(i).Cuneate_spikes(downMove(i).idx_peak_speed-(before*100):downMove(i).idx_peak_speed+(after*100),num1);
    end
    ...
    for  i = 1:length(rightMove)
        rightBumpFiring(i,:) = rightMove(i).Cuneate_spikes(rightMove(i).idx_peak_speed-(before*100):rightMove(i).idx_peak_speed+(after*100),num1);
    end
    ...
    for  i = 1:length(leftMove)
        leftBumpFiring(i,:) = leftMove(i).Cuneate_spikes(leftMove(i).idx_peak_speed-(before*100):leftMove(i).idx_peak_speed+(after*100),num1);
    end
