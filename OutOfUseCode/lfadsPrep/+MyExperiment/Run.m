classdef Run < LFADS.Run
    methods
        ...
        function out = generateCountsForDataset(r, dataset, mode, varargin) %#ok<INUSL,INUSD>
            ...
            trimmedTD = data.td;
            fields = fieldnames(trimmedTD);
            disp(fields)
            str = input('Enter the name of the field to LFADS', 's');
            
            spikes = cat(3, trimmedTD.(str));
            spikes = permute(spikes, [3,2,1]);
            
            out.counts = spikes;
            
            timeMs = linspace(trimmedTD(1).bin_size,  trimmedTD(1).bin_size*length(spikes(1,1,:)), length(spikes(1,1,:)));
            out.timeVecMs = timeMs.*1000;
            
            conditionMask = generateConditionMask(trimmedTD);
            out.conditionId = conditionMask;
