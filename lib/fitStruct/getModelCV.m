...
if add_pred_to_td
    ...
    for trial = 1:length(trainTD)
        ...
        trainTD(trial).([td_fn_prefix '_' model_name]) = yfit;
        
    end
    for trial = 1:length(testTD)
        x  = get_vars(testTD(trial),in_signals);
        y  = get_vars(testTD(trial),out_signals);
        ...
        testTD(trial).([td_fn_prefix '_' model_name]) = yfit;
