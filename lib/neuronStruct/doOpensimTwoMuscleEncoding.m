function neurons1 = doOpensimTwoMuscleEncoding(td, params)
...
%               disp(['Working on', encVars{v}])         
              switch encVars{v}
                ...
                case 'handElb'
                  ...
                  otherwise
                    if contains(encVars{v}, 'X')
                      musc = split(encVars{v}, 'X');
                      varIn = [muscLen(:,find(strcmp(singMuscVar, musc{1}))), muscVel(:,find(strcmp(singMuscVar, musc{1}))), muscLen(:,find(strcmp(singMuscVar, musc{2}))), muscVel(:,find(strcmp(singMuscVar, musc{2})))];
                      varInTe = [muscLenTe(:,find(strcmp(singMuscVar, musc{1}))), muscVelTe(:,find(strcmp(singMuscVar, musc{1}))), muscLenTe(:,find(strcmp(singMuscVar, musc{2}))), muscVelTe(:,find(strcmp(singMuscVar, musc{2})))];
                    end
              end     
...
%                neurons(u).(encVars{v})(f) = corr(predict(lm1, varInTe), frTe(:,u))^2;
   ...
   nPath = [path, filesep, 'neuronStruct', filesep, 'RefFrameTwoMuscleEnc', filesep];
   name = [td(1).monkey, '_', td(1).date, '_', td(1).task, '_RefFrameTwoMuscleEnc.mat'];
   mkdir(nPath)
