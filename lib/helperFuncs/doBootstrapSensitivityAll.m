function neurons = doBootstrapSensitivityAll(neurons, td, params)
    ...
    for boot = 1:numBoots
        ...
        for i = 1:length(guide(:,1))  
            ...
            if windowInds
                neurons.sActAllBoot(unitNum, boot) = sAct(i,boot);
                neurons.sPasAllBoot(unitNum, boot) = sPas(i,boot);
                neurons.sDifAllBoot(unitNum, boot) =sAct(i,boot) - sPas(i,boot);
            else
                neurons.sActAllBootNoWin(unitNum, boot) = sAct(i,boot);
                neurons.sPasAllBootNoWin(unitNum, boot) = sPas(i,boot);
                neurons.sDifAllBootNoWin(unitNum, boot) =sAct(i,boot) - sPas(i,boot);
            end
        end
    end
end