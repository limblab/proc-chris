function neurons = doBootstrapSensitivityAllSameZ(neurons, td, params)
    ...
    for boot = 1:numBoots
        ...
        inputs = [posAct,velAct, accAct;posPas,velPas, accPas];
        ...
        for i = 1:length(guide(:,1))  
            ...
            sActAllPos(i, boot) = norm(lmAct.Coefficients.Estimate(2:3));
            sActAllVel(i, boot) = norm(lmAct.Coefficients.Estimate(4:5));
            sActAllAcc(i, boot) = norm(lmAct.Coefficients.Estimate(6:7));

            lmPas = fitlm(inputsPas, pasFR(:,i));
            sPasAllPos(i, boot) = norm(lmPas.Coefficients.Estimate(2:3));
            sPasAllVel(i, boot) = norm(lmPas.Coefficients.Estimate(4:5));
            sPasAllAcc(i, boot) = norm(lmPas.Coefficients.Estimate(6:7));
            if windowInds
                neurons.sActAllBootPos(unitNum, boot) = sAct(i,boot);
                neurons.sPasAllBootPos(unitNum, boot) = sPas(i,boot);
                neurons.sDifAllBootPos(unitNum, boot) =sAct(i,boot) - sPas(i,boot);
                
                neurons.sActAllBootVel(unitNum, boot) = sAct(i,boot);
                neurons.sPasAllBootVel(unitNum, boot) = sPas(i,boot);
                neurons.sDifAllBootVel(unitNum, boot) =sAct(i,boot) - sPas(i,boot);
                
                neurons.sActAllBootAcc(unitNum, boot) = sAct(i,boot);
                neurons.sPasAllBootAcc(unitNum, boot) = sPas(i,boot);
                neurons.sDifAllBootAcc(unitNum, boot) =sAct(i,boot) - sPas(i,boot);
            else
