function [nNeg, nPos, nNoDif, proj1, statTab] = plotSensitivityAll(neurons, params)
    ...
    if useWindowed
        sDif = neurons.sDifAllBoot;
    else
        sDif = neurons.sDifAllBootNoWin;
    end
    ...
    for mon = 1:length(monks)
        ...
        if useWindowed
            sAct = neuronsM.sActAllBoot;
            sPas = neuronsM.sPasAllBoot;
        else
    ...
    if useMins
        if useWindowed
            sAct = neurons.sActAllBoot;
            sPas = neurons.sPasAllBoot;
        else
