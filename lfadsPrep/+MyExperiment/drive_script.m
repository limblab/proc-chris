...
load('/media/chris/HDD/Data/MonkeyData/Butter/Butter_CO_20180329_4_TD_sorted_FirstSort.mat')
params.runName = 'ButterCuneateLFADS';

params.monkey = 'Butter';
params.date = '20180329';
datasetPath = saveLFADSdatasetsCObump(td, params);

%% Locate and specify the datasets
dc = MyExperiment.DatasetCollection(datasetPath);
dc.name = 'ButterCuneateLfads';
...
MyExperiment.Dataset(dc, 'Butter_20180329_td_peak.mat');


dc.datasets(1,1).comment = 'Butter data binned +/- 200 ms around peak';
...
rc = MyExperiment.RunCollection(runRoot, 'ButterLFADS', dc);
...
par.c_batch_size = 25; % must be < 1/5 of the min trial count
...
par.c_learning_rate_stop = 1e-4; % we can stop really early for the demo
